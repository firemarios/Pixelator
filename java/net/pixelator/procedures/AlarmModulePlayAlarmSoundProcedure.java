			...
			if (PixelatorModVariables.MapVariables.get(world).alarm_timer % 20 == 0) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("pixelator:alarm")), SoundSource.BLOCKS, 10, 1);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("pixelator:alarm")), SoundSource.BLOCKS, 10, 1, false);
					}
				}
			}
