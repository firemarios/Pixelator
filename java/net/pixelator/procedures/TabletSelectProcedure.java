			...
			PixelatorMod.queueServerWork(1, () -> {
				if (!((new Object() {
					public String getResult(LevelAccessor world, Vec3 pos, String _command) {
						...
						("tag @e[type=interaction,tag=" + (entity.getCapability(PixelatorModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new PixelatorModVariables.PlayerVariables())).teleportname + "]" + " list"))).contains("private"))) {
					if (world instanceof ServerLevel _level)
